...
export const BYOK_CHANGED_EVENT = 'srujan-byok-changed';
...
export function TerminalChat({
    ...
    // BYOK status (edited in the connect bar ABOVE the chat; this is read-only)
    ...
    useEffect(() => {
        const refresh = () => setByokSaved(getByokConfig());
        refresh();
        ...
        <div className={`flex flex-col h-full w-full max-w-full bg-bg-base/80 backdrop-blur-sm border border-cyan-900/30 rounded-lg overflow-hidden ${className}`}>
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-bg-surface border-b border-cyan-900/30">
                ...
                <div className="ml-auto flex items-center gap-3">
                    {/* BYOK status chip (edit in the connect bar above the chat) */}
                    <span
                        className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs font-mono border ${byokSaved
                            ? 'bg-emerald-400/15 text-emerald-400 border-emerald-400/40'
                            : 'bg-amber-400/15 text-amber-400 border-amber-400/40 animate-pulse'
                            }`}
                        title={byokSaved ? `Connected: ${byokSaved.provider}` : 'Connect your API key in the bar above'}
                    >
                        <span>🔑</span>
                        <span className="hidden sm:inline">
                            {byokSaved ? BYOK_PROVIDERS.find(p => p.id === byokSaved.provider)?.label || byokSaved.provider : 'No key — use bar above'}
                        </span>
                    </span>
                    {/* Voice Toggle Button */}
                    {isSupported && (
                    ...
                    <div className="font-mono text-sm p-3 sm:p-4 border border-cyan-500/30 rounded-lg bg-cyan-500/5">
                        ...
                        {!byokSaved && (
                            <p className="mt-3 text-amber-400/90 text-[11px] sm:text-xs leading-relaxed border-t border-cyan-900/30 pt-3">
                                ⚡ This chat runs on <b>your</b> API key — use the <span className="px-1.5 py-0.5 bg-amber-400/15 border border-amber-400/40 rounded">connect bar above the chat</span>,
                                pick your favourite AI provider (Gemini &amp; Groq are free), paste a key, and start talking.
                            </p>
                        )}
                    </div>
                )}
