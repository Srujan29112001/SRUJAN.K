...
export function VideoTransition() {
    ...
    const completeTransition = useCallback(() => {
        console.log('VideoTransition: Completing, target:', targetScrollRef.current);
    ...
    const playVideo = useCallback((direction: 'down' | 'up') => {
        ...
        if (direction === 'down') {
            ...
            if (projects) {
                targetScrollRef.current = projects.offsetTop;
                console.log('VideoTransition: DOWN target =', targetScrollRef.current);
            } else {
                console.error('Cannot find #projects');
                return;
            }
        } else {
            // Scroll UP - go to footer/exit of Skills section
            // Use getBoundingClientRect for accurate position (offsetTop is relative to parent)
            const skillsContent = document.getElementById('skills-content');

            if (skillsContent) {
                // Get actual position in document using getBoundingClientRect
                const rect = skillsContent.getBoundingClientRect();
                const skillsContentBottom = rect.bottom + window.scrollY;

                // Scroll to position where bottom of skills-content is near bottom of viewport
                targetScrollRef.current = skillsContentBottom - window.innerHeight + 100;

                console.log('VideoTransition: UP target =', targetScrollRef.current);
                console.log('Debug: rect.bottom=', rect.bottom, 'scrollY=', window.scrollY, 'skillsContentBottom=', skillsContentBottom);
            } else {
                console.error('Cannot find #skills-content');
                return;
            }
        }
