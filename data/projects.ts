...
export const projects: Project[] = [
  {
    ...
    "category": "AI",
    "metric": "9-Agent AutoML",
    "description": "Capstone (IIIT-H GenAI). Upload a CSV and ask in plain English — nine AI agents plan, write & self-correct code, model, explain, visualize, pull live web context, and report, end to end.",
    "longDescription": "Helix turns \"a CSV + a plain-English question\" into a business-ready answer automatically — the capstone project from my IIIT Hyderabad × TalentSprint Advanced GenAI & Prompt Engineering certification. It attacks the organizational \"data bottleneck\": scarce data scientists, the 60–70% of analyst time lost to cleaning, and brittle code that breaks on iteration.\n\nARCHITECTURE — two parts. The frontend is a Next.js / React \"Studio\" (Tailwind, Framer Motion, canvas visualizations) where users upload a dataset, state a goal, and watch the agents work in real time over Server-Sent Events. The backend is a FastAPI + LangGraph multi-agent orchestration layer modeled as a directed state graph: agents run sequentially with conditional looping for self-correction.\n\nTHE NINE AGENTS — (1) Planner decomposes the goal into an ordered analysis plan via LLM chain-of-thought; (2) Coder writes Python for each step, grounded by RAG so it does not hallucinate APIs; (3) Executor runs the code in a sandbox and captures stdout/errors; (4) Critic reads tracebacks, rewrites the code, and retries (up to 5 attempts) — the self-healing loop; (5) AutoML searches models and hyperparameters with FLAML; (6) Explainer quantifies what drives predictions, and in which direction, using SHAP; (7) Visualizer picks the best chart for each finding; (8) Researcher pulls live real-world context from the web to ground the narrative; (9) Reporter writes the business story and recommendation.\n\nWORKFLOW — auto-detects the task type (classification / regression / clustering / NLP), cleans messy data, builds and tunes a model, surfaces the key drivers, visualizes them, and produces a report — without human intervention. Code generation is RAG-grounded via ChromaDB + MiniLM embeddings over a recipe corpus; execution is sandboxed with RestrictedPython by default or an isolated E2B micro-VM. The LLM layer is provider-pluggable — Groq, OpenAI, DeepSeek, Mistral, Google Gemini, or Claude.\n\nIt is domain-agnostic (finance, retail, healthcare, marketing) and built for business teams and analysts who need fast insight from tabular data without writing code. Live app: helix-henna.vercel.app.",
    "tech": [
      "LangGraph",
      "FastAPI",
      "Next.js",
