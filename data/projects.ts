...
export const projects: Project[] = [
  // ==================== AI PROJECTS ====================
  {
    ...
    tech: ['LangGraph', 'FastAPI', 'Next.js', 'LangChain', 'DeepSeek-Coder', 'Mistral-7B', 'FLAML', 'SHAP', 'ChromaDB', 'RAG', 'RestrictedPython', 'Python', 'TypeScript'],
  ...
  },
  {
    ...
    longDescription: 'A fine-grained image classifier that distinguishes daisies from dandelions using a Swin Transformer — a hierarchical vision transformer whose shifted-window self-attention captures both local petal texture and global flower shape. Workflow: images are resized and normalized, then augmented (flips, rotation, colour jitter) to fight overfitting on a small two-class set; a pretrained Swin backbone is fine-tuned with a lightweight classification head and trained in JupyterLab with GPU acceleration; the model is evaluated on a held-out split via accuracy and a confusion matrix. Stack: PyTorch, Swin Transformer, torchvision.',
  ...
  },
  {
    ...
    longDescription: 'A binary facial-emotion classifier (happy vs sad) built end to end. Workflow: a custom image set is cleaned of corrupt files, normalized to [0,1] and batched through an image data pipeline; a convolutional neural network — stacked Conv2D + max-pool blocks feeding dense layers with a sigmoid output — is trained with binary cross-entropy while loss/accuracy curves are monitored to catch overfitting; the model is then scored with precision, recall and accuracy and used for single-image inference. Stack: TensorFlow/Keras, OpenCV, Matplotlib.',
  ...
  },
  {
    ...
    longDescription: 'A real-time push-up rep counter and form checker driven by pose estimation. Workflow: MediaPipe Pose extracts 33 body landmarks from the webcam feed via OpenCV; joint angles (shoulder–elbow–wrist for depth, plus the hip–shoulder–ankle line for posture) are computed with vector math; a small state machine counts a rep on each down→up transition past angle thresholds, while the side view flags broken form such as sagging hips. Live overlays draw the skeleton, the current angle, the rep count and form feedback. Stack: MediaPipe, OpenCV, NumPy, Python.',
  ...
  },
  {
    ...
    longDescription: 'A real-time bicep-curl counter using markerless pose estimation. Workflow: MediaPipe Pose tracks the shoulder, elbow and wrist landmarks each frame; the elbow-flexion angle is computed from the dot-product of the two limb vectors; a two-stage state machine (arm extended ↔ arm flexed, gated by upper/lower thresholds with hysteresis to reject jitter) increments the count on every completed curl. The webcam feed is annotated with the live angle, the current stage and the rep total. Stack: MediaPipe, OpenCV, NumPy, Python.',
  ...
  },
  {
    ...
    longDescription: 'A touchless human–computer interface that maps a hand gesture to system volume. Workflow: MediaPipe Hands detects 21 hand landmarks from the webcam; the Euclidean distance between the thumb tip and index-finger tip is measured every frame and linearly mapped (with interpolation and clamping) onto the OS volume range, which is then set through the system audio API / PyAutoGUI. A live overlay renders the landmarks, the pinch line and a volume bar for feedback. Stack: MediaPipe, OpenCV, NumPy, PyAutoGUI, Python.',
  ...
  },
  {
    ...
    longDescription: 'A browser-native real-time pose-estimation app with no server-side inference. Workflow: a React app requests the webcam via getUserMedia and renders frames to a canvas; TensorFlow.js loads the PoseNet model and runs inference fully client-side on each animation frame, returning 17 keypoints with confidence scores; keypoints above a confidence threshold are drawn as joints and connected into a live skeleton overlay. Stack: React, TensorFlow.js, PoseNet, HTML Canvas, getUserMedia.',
  ...
  },
  {
    ...
    longDescription: 'A real-time person-segmentation demo running entirely in the browser. Workflow: a React app streams the webcam to a canvas and uses TensorFlow.js BodyPix to perform per-pixel segmentation each frame, producing a body-part mask; the mask is composited over the video (coloured part-map or background blur) on a second canvas via requestAnimationFrame. All inference runs client-side on the WebGL backend, so no frames ever leave the device. Stack: React, TensorFlow.js, BodyPix, HTML Canvas.',
  ...
  },
  {
    ...
    longDescription: 'A client-side real-time hand-tracking app. Workflow: a React app captures the webcam and, on every animation frame, runs TensorFlow.js\'s handpose model to predict 21 3D hand landmarks plus a bounding box; the landmarks and finger connections are rendered as an overlay on a canvas aligned to the video. Inference runs on the in-browser WebGL backend for low latency and full privacy. Stack: React, TensorFlow.js, Handpose, HTML Canvas.',
